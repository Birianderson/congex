        ...
        <form id="frm" name="frm" data-method="post" :action="acao">
            ...
            <div class="row">
                <div class="col-6 mb-4">
                    ...
                    <input type="text" class="form-control" id="exercicio" name="exercicio" v-model="info.exercicio" />
                </div>
                <div class="col-6 mb-4">
                    <label for="data_vigencia" class="form-label" :class="{ 'required': !readOnly }">Data da Vigência</label>
                    <input type="date" class="form-control" id="data_vigencia" name="data_vigencia" v-model="info.data_vigencia"/>
                </div>
            </div>
            <div class="row">
                <div class="col-6 mb-4">
                    <label for="termo_de_referencia" class="form-label" :class="{ 'required': !readOnly }">Termo de Referência</label>
                    <input type="text" class="form-control" id="termo_de_referencia" name="termo_de_referencia" v-model="info.termo_de_referencia"/>
                </div>
                <div class="col-6 mb-4">
                    <label for="empenho" class="form-label" :class="{ 'required': !readOnly }">Número do Empenho</label>
                    <input type="text" class="form-control" id="empenho" name="empenho" v-model="info.empenho"/>
                </div>
            </div>
            <div class="row">
                <div class="col-6 mb-4">
                    <label for="valor_empenho" class="form-label" :class="{ 'required': !readOnly }">Valor Empenhado</label>
                    <input-money id="valor_empenho" name="valor_empenho" :value="info.valor_empenho"></input-money>
                </div>
                <div class="col-6 mb-4">
                    <label for="valor_liquidacao" class="form-label" :class="{ 'required': !readOnly }">Valor Liquidado</label>
                    <input-money id="valor_liquidacao" name="valor_liquidacao" :value="info.valor_liquidacao"></input-money>
                </div>
            </div>
...
export default {
    setup(props, { emit }) {
        ...
        const loadData = async () => {
            try {
                const response = await axios.get(`${acao.value}${props.data.id_empenho}`);
                acao.value += props.data.id_empenho;
